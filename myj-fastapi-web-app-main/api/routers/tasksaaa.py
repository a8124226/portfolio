...
import api.cruds.tasksaaa as task_crud
...
def list_tasks(
    ...
    return task_crud.get_multiple_tasks_with_done(db,user_id=current_user.get("id"))


@router.get("/tasksaaa/{task_id}")
def get_task(
    task_id: int,
    db: Session = Depends(get_db),
    current_user: dict = Depends(get_current_user),
):
    tasksaaa = task_crud.get_task_with_done(db, task_id=task_id)
    ...
    if tasksaaa.get("user_id") != current_user.get("id"):
        raise HTTPException(status_code=403, detail="Forbidden")

    return tasksaaa


@router.put("/tasksaaa/{task_id}")
def update_task(
    task_id: int,
    task_body: dict = Body(),
    db: Session = Depends(get_db),
    ...
    tasksaaa = task_crud.get_task(db, task_id=task_id)
...
@router.put("/tasksaaa/{task_id}/image")
...
@router.delete("/tasksaaa/{task_id}", response_model=None)
def delete_task(
    task_id: int,
    db: Session = Depends(get_db),
    current_user: dict = Depends(get_current_user),
    ...
    # ✅ 修正ポイント：正しい変数名に変更
    return task_crud.delete_task(db, original=tasksaaa)
